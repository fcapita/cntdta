        ...
        <div className="max-w-md w-full bg-white rounded-3xl shadow-2xl p-10">
          <div className="text-center mb-10">
            <div className="w-20 h-20 bg-blue-100 rounded-2xl flex items-center justify-center mx-auto mb-6">
              <i className="fa-solid fa-user-lock text-3xl text-blue-600"></i>
            </div>
            <h1 className="text-2xl font-bold text-gray-900">Portal do Profissional</h1>
            <p className="text-gray-500">Introduza os seus dados para aceder à área reservada.</p>
          </div>
          
          <form className="space-y-6" onSubmit={(e) => { e.preventDefault(); setIsLogged(true); }}>
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">Número da Cédula / E-mail</label>
              <input type="text" className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-blue-500 outline-none transition-all" placeholder="Ex: CNT/1234/2024" required />
            </div>
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">Palavra-passe</label>
              <input type="password" className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-blue-500 outline-none transition-all" placeholder="••••••••" required />
            </div>
            <button type="submit" className="w-full bg-blue-600 hover:bg-blue-700 text-white py-4 rounded-xl font-bold shadow-lg transition-all hover:scale-[1.02]">
              Entrar no Portal
            </button>
          </form>
          
          <div className="mt-8 pt-8 border-t border-gray-100 flex flex-col gap-4 text-center">
            <a href="#" className="text-sm text-blue-600 hover:underline">Esqueceu a palavra-passe?</a>
            <p className="text-sm text-gray-500">Ainda não tem conta? <a href="#" className="text-blue-600 font-bold">Solicitar Inscrição</a></p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="animate-fadeIn max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="flex flex-col md:flex-row justify-between items-center mb-12 gap-6">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Bem-vindo ao seu Portal</h1>
          <p className="text-gray-500">Aceda aos serviços integrados da Zulumuka para o CNTDTA.</p>
        </div>
        <button onClick={() => setIsLogged(false)} className="bg-red-50 text-red-600 px-6 py-2 rounded-full font-bold hover:bg-red-100 transition-all flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {services.map((service, i) => (
          <div key={i} className="bg-white p-8 rounded-3xl shadow-lg border border-gray-100 hover:shadow-2xl transition-all cursor-pointer group">
            ...
            <div className="flex items-center text-blue-600 font-bold gap-2 text-sm group-hover:gap-4 transition-all">
